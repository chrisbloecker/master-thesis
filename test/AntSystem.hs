...
decIter :: Configuration -> BasicProcess Configuration
decIter conf@Configuration {..} = return conf { iterations = iterations - 1 }

stopIter :: Configuration -> BasicProcess Bool
stopIter Configuration {..} = return (iterations > 0)
...
decIterProcess = Simple decIter

stopIterProcess :: Predicate Configuration
stopIterProcess = Simple stopIter
...
interpret conf@Configuration {..} = do
  let antRuns   = Multilel (replicate ants antProcess) Id combinePathsProcess
      innerProc = antRuns `Sequence` evaporationProcess `Sequence` decIterProcess
      loop      = Repetition stopIterProcess innerProc
