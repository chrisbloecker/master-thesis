...
\begin{itemize}
  \item $\choice{R}{P}{Q}$ \hspace*{2.7em} (Choice)\index{Operator!choice}
  \item $\parallel{R}{P}{Q}$ \hspace*{2.8em} (Parallel)\index{Operator!parallel}
  \item $\sequence{P}{Q}$ \hspace*{2.2em} (Sequence)\index{Operator!sequence}
...
\nomenclature{$f_P$}{Intrinsic function of basic process $P$}
...
\label{exp:sem_chice}
Consider the definitions of $\sigma, \delta, S$ and $D$ from \expref{exp:sem_atomic}. If we compose $S$ and $D$ using the choice operator $\choiceop$, the semantics of the resulting process is as follows:
  \begin{equation}
...
Consider the definitions of $\sigma, \delta, S$ and $D$ from \expref{exp:sem_atomic} and use the sequence operator $\sequenceop$ to compose $D$ and $S$. Since $\sequenceop$ does not have commutative properties, $S \sequenceop D$ and $D \sequenceop S$ yield different results.
  \begin{equation}
