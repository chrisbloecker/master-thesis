...
Different programming languages take different approaches for expressing parallelism and the ideas from process calculi. E.g. \textsf{occam} shows a very close resemblance of \textsc{CSP} and different \textsf{Haskell} modules present different ways of expressing parallelism.
...
Assume there are operations \texttt{a}, \texttt{b}, \texttt{c} where \texttt{a} and \texttt{b} take an \texttt{MVar} and can be executed concurrently. \texttt{c} consumes the results produced by \texttt{a} and \texttt{b} and returns a result. Then, using \texttt{Concurrent Haskell}, this can be expressed as shown in \lstref{lst:example_concurrent_haskell}.
...
The \texttt{Eval} monad introduces two operations: \texttt{rpar} and \texttt{rseq}. \texttt{rpar} is used to create a computation that can be run in parallel while \texttt{rseq} is used to create a computation that is executed sequentially.