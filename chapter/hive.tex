...
Now we need to pass the list of names of all the functions that shall be called remotely to the \textsf{Cloud Haskell} function \texttt{remotable}. Their names will then be entered into a table of static code pointers 
\begin{lstlisting}[language=Haskell, caption=., numbers=left, frame=bt, firstnumber=28]
remotable ['val, 'add, 'subtract, 'multiply, 'divide, 'intDict]
\end{lstlisting}

\begin{lstlisting}[language=Haskell, caption=., numbers=left, frame=bt, firstnumber=29]
...
\begin{lstlisting}[language=Haskell, caption=., numbers=left, frame=bt, firstnumber=43]
